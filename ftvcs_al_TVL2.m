function [U, out] = ftvcs_al_TVL2(A,b,p,q,r,opts)
%
...
if opts.TVnorm == 1
    ...
    lam1 = sum(sum(abs(Wx) + abs(Wy) + abs(Wz)));
else
    ...
    Wy = S.*Uy;
    Wz = S.*Uz;
    lam1 = sum(sum(sqrt(Wx.*conj(Wx) + Wy.*conj(Wy) + Wz.*conj(Wz))));
end

[lam2,lam3,lam4,f,g2,Au,g] = get_g(U,Ux,Uy,Uz,Wx,Wy,Wz,lam1,beta,mu,A,b,...
    Atb,sigmax,sigmay,sigmaz); ??
%lam, f: constant      g2: pq (=n)        Au: m         g: pq
...
for ii = 1:opts.maxit
    ...
    if ~isempty(gp)
        ...
        dDd = norm(dx,'fro')^2 + norm(dy,'fro')^2 + norm(dz,'fro')^2;      % dDd: cosntant
        Ad = A(d,1);                        %Ad: m
    ...
    [lam2,lam3,lam4,f,g2,Au,g] = get_g(U,Ux,Uy,Uz,Wx,Wy,Wz,lam1,...
        beta,mu,A,b,Atb,sigmax,sigmay,sigmaz);
    ...
    while f > C - alpha*const
        if cnt == 5
            ...
            dDd = norm(dx,'fro')^2 + norm(dy,'fro')^2 + norm(dz,'fro')^2;
            Ad = A(d,1);
            tau = abs((d'*d)/(dDd + muDbeta*(Ad)'*Ad));
            U = Up(:) - tau*d;
            % projected gradient method for nonnegtivity
        ...
        [U,lam2,lam3,lam4,f,Ux,Uy,Au,g,g2] = update_g(p,q,lam1,...
            alpha,beta,mu,Up,du,gp,dg,g2p,dg2,Aup,dAu,Wx,Wy,Wz,Uxp,dUx,...
            Uyp,dUy,Uzp,dUz,b,sigmax,sigmay,sigmaz);??
        cnt = cnt + 1;
    end
    ...
    if alpha ~= 0
        ...
        [lam1,lam2,lam4,f,g2] = update_W(beta,Wx,Wy,Wz,Ux,Uy,Uz,sigmax,...
            sigmay,sigmaz,lam1,lam2,lam4,f,opts.TVnorm);
    end
    ...
    if RelChg < tol_inn  && ~fst_itr
        ...
        % update multipliers
        [sigmax,sigmay,sigmaz,lam4,~] = update_mlp(beta,Wx,Wy,Wz,Ux,Uy,Uz,...
...
function [lam2,lam3,lam4,f,g2,Au,g] = get_g(U,Ux,Uy,Uz,Wx,Wy,Wz,lam1,...
    beta,mu,A,b,Atb,sigmax,sigmay,sigmaz)
global Dt
...
Vy = Uy - Wy;
Vz = Uz - Wz;
lam2 = sum(sum(Vx.*conj(Vx) + Vy.*conj(Vy)+ Vz.*conj(Vz)));
...
%lam4
lam4 = sum(sum(conj(sigmax).*Vx + conj(sigmay).*Vy + conj(sigmaz).*Vz));
...
function [U,lam2,lam3,lam4,f,Ux,Uy,Uz,Au,g,g2] = update_g(p,q,lam1,...
    alpha,beta,mu,Up,du,gp,dg,g2p,dg2,Aup,dAu,Wx,Wy,Wz,Uxp,dUx,Uyp,dUy,Uzp,dUz,b,...
...
if option == 1
    ...
    lam1 = sum(sum(sqrt(Wx.^2 + Wy.^2 + Wz.^2)));
end
Vx = Ux - Wx;
Vy = Uy - Wy;
Vz = Uz - Wz;
...
function [sigmax,sigmay,sigmaz,lam4,f] = update_mlp(beta, ...
    Wx,Wy,Wz,Ux,Uy,Uz,sigmax,sigmay,sigmaz,lam4,f)
